...
const inputFilter = useCallback((values:any)=>{
    settodolist( todolist.filter((item,index)=>{
        return item.content.includes(values)
    }) )
},[]) 

const splicelist = useCallback((index:any)=>{
 ...
 interface Cb<K, V> {
    ...
    return(
        <>
            ...
            <Search addInput={addInput} inputFilter= {inputFilter} addItem ={addItem}></Search>
            <Content todolist={todolist} splicelist={splicelist}>
                <TodoItem splicelist={splicelist}></TodoItem>
            </Content>
            {
